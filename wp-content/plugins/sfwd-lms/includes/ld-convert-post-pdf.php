...
if ( ! function_exists( 'learndash_purchase_invoice_pdf' ) ) {
	...
	function learndash_purchase_invoice_pdf( $pdf_content = array() ) {
		...
		// Just to ensure we have valid IDs.
		$purchase_invoice_args['post_id'] = absint( $pdf_content['purchase_id'] );
		...
		if ( ! empty( $learndash_pdf_username ) ) {
			if ( ! empty( $purchase_invoice_args['pdf_title'] ) ) {
				$purchase_invoice_args['pdf_title'] .= " $sep ";
			}
			$purchase_invoice_args['pdf_title'] .= $purchase_invoice_args['user_id'];
		}
		 ...
		 * @param array $query_string_params_supported Array of supported query string params.
		 */
		$query_string_params_allowed = apply_filters( 'learndash_purchase_invoice_query_string_params_allowed', $query_string_params_allowed, $query_string_params_supported );
		...
		if ( in_array( 'ratio', $query_string_params_allowed, true ) ) {
			if ( ! empty( $_GET['ratio'] ) ) {
				$purchase_invoice_args['ratio'] = floatval( $_GET['ratio'] );
			}
		}
		 ...
		 */
		if ( ! defined( 'LEARNDASH_TCPDF_LEGACY_LD322' ) ) {
			$use_LD322_define = apply_filters( 'learndash_tcpdf_legacy_ld322', true, $purchase_invoice_args ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase

			/**
			 * Define LearnDash LMS - Set to enable legacy TCPDF processing logic.
			 *
			 * LD 3.2.0 includes an upgrade of the TCPDF library for generating PDF
			 * purchase_invoices. The newer TCPDF library incldues some improvements which
			 * cause the rendering to not match the prior version of the library. This
			 * define if set to `true` will enable legacy logic in the new library.
			 *
			 * @since 3.2.2
		...
		$post_pricing = $pdf_content['purchase_price'];
		...
		$subscription_details = $post_pricing['price'];
		if ( ! empty( $post_pricing['interval'] ) ) {
			$subscription_details .= ' every ' . $post_pricing['interval'] . ' ' . $post_pricing['frequency'];
		}
		if ( ! empty( $post_pricing['repeat_frequency'] ) ) {
			$subscription_details .= ' for ' . $post_pricing['repeats'] * $post_pricing['interval'] . ' ' . $post_pricing['frequency'];
		}

		$trial_details = $post_pricing['trial_price'] . ' for ' . $post_pricing['trial_interval'] . ' ' . $post_pricing['trial_frequency'];

		$purchase_invoice_content  = $pdf_content['purchaser_name'] . '<br />';
		$purchase_invoice_content .= $pdf_content['purchase_id'] . '<br />';
		if ( ! empty( $post_pricing['trial_price'] ) ) {
			$purchase_invoice_content .= $trial_details . '<br />';
		}
		$purchase_invoice_content .= $subscription_details . '<br />';
		$purchase_invoice_content .= $pdf_content['vat_number'] . '<br />';
		$purchase_invoice_content .= $pdf_content['company_name'] . '<br />';
		$purchase_invoice_content .= $pdf_content['company_address'] . '<br />';
		$purchase_invoice_content .= $pdf_content['company_logo'];
		...
		// Add width and height into image tag.
		$purchase_invoice_content = preg_replace_callback( "/(<img[^>]*?src=['\"]((http:\/\/|https:\/\/|\/)[^'\"]*?(jpg|jpeg|gif|png))['\"])([^>]*?>)/i", 'learndash_post2pdf_conv_img_size', $purchase_invoice_content );

		if ( ( ! defined( 'LEARNDASH_TCPDF_LEGACY_LD322' ) ) || ( true !== LEARNDASH_TCPDF_LEGACY_LD322 ) ) {
			$purchase_invoice_content = wpautop( $purchase_invoice_content );
		}
		...
		if ( ( defined( 'LEARNDASH_TCPDF_LEGACY_LD322' ) ) && ( true === LEARNDASH_TCPDF_LEGACY_LD322 ) ) {
			$purchase_invoice_content = preg_replace( '/\n/', '<br/>', $purchase_invoice_content ); // "\n" should be treated as a next line.
		}
		 ...
		 * @since 4.1.0
		 *
		 * @param boolean $include_purchase_invoice_styles Whether to include purchase_invoice styles.
		 * @param int     $purchase_invoice_id                   purchase_invoice post ID.
		 */
		if ( apply_filters( 'learndash_purchase_invoice_styles', true, $purchase_invoice_args['purchase_invoice_id'] ) ) {
			$purchase_invoice_styles = LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_purchase_invoices_Styles', 'styles' );
			$purchase_invoice_styles = preg_replace( '/<style[^>]*?>(.*?)<\/style>/is', '$1', $purchase_invoice_styles );
			if ( ! empty( $purchase_invoice_styles ) ) {
				$purchase_invoice_content = '<style>' . $purchase_invoice_styles . '</style>' . $purchase_invoice_content;
			}
		}
		...
		// get featured image.
		$img_file = learndash_get_thumb_path( $purchase_invoice_args['purchase_invoice_id'] );
